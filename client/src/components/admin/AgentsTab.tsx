            ...
            return (
              <div key={agent.id} className="border border-border rounded-lg p-4">
                ...
                <div className="flex items-center gap-1 mb-3">
                  {[1, 2, 3, 4, 5].map(s => (
                    <span key={s} className={`text-lg ${s <= rating ? 'text-yellow-400' : 'text-muted-foreground/50'}`}>&#9733;</span>
                  ))}
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {agent.languages && (
                    <p className="text-muted-foreground">Languages: {agent.languages}</p>
                  )}
                </div>
                <div className="mt-3">
      ...
      {agents && agents.length > 0 && (() => {
        ...
        return (
          <div className="mt-6 bg-card border border-border rounded-xl p-4 md:p-6">
            <h4 className="text-sm md:text-base font-semibold text-foreground mb-4">Weekly Availability</h4>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse min-w-[500px]">
                <thead>
                  <tr>
                    <th className="text-left py-2 px-3 text-xs md:text-sm font-semibold text-muted-foreground border-b border-border w-32">Agent</th>
                    {days.map((d, i) => {
                      const isToday = d.toDateString() === today.toDateString();
                      return (
                        <th key={i} className={`py-2 px-2 text-xs md:text-sm text-center border-b border-border ${isToday ? 'bg-primary/5 font-bold text-primary' : 'font-semibold text-muted-foreground'}`}>
                          <div>{dayLabels[i]}</div>
                      ...
                      <tr key={agent.id} className="border-b border-border/50">
                        ...
                        {days.map((d, i) => {
                          ...
                          if (agent.status === 'inactive') {
                            bgColor = 'bg-gray-100'; textColor = 'text-muted-foreground'; label = 'Inactive';
