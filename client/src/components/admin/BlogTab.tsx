...
import { Eye, Sparkles, Send, Loader2, Clock, Calendar } from "lucide-react";
...
export function BlogTab() {
  ...
  /** Determine post status from isPublished flag and publishedAt date */
  const getPostStatus = (
    post: BlogPost
  ): "draft" | "scheduled" | "published" => {
    if (post.isPublished !== 1) return "draft";
              ...
              <div className="p-3 md:p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1.5 items-center">
                      {(() => {
                        ...
                        return (
                          <span
                            className={`px-2 py-0.5 rounded text-xs font-medium inline-flex items-center gap-1 ${config.className}`}
                          >
                            {status === "scheduled" && (
                ...
                <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-border/50">
                  ...
                  <button
                    onClick={() => {
                      setEditingBlog(post);
                      // For scheduled posts, pre-fill the scheduledAt in datetime-local format
                      ...
                      if (
                        ...
                        scheduledAtValue = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}T${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
                      }
                      setBlogForm({
                        title: post.title,
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name="publishMode"
                    checked={blogForm.isPublished && !blogForm.scheduledAt}
                    onChange={() =>
                      setBlogForm(p => ({
                        ...p,
                        isPublished: true,
              ...
              {blogForm.isPublished && blogForm.scheduledAt && (
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setBlogForm(p => ({ ...p, scheduledAt: e.target.value }))
                    }
                    min={new Date().toISOString().slice(0, 16)}
                    className="px-3 py-2 border border-border rounded-lg text-sm"
                  />
                  {blogForm.scheduledAt && (
                    <p className="text-xs text-muted-foreground mt-1">
                      Post will go live on{" "}
                      {new Date(blogForm.scheduledAt).toLocaleString()}
                    </p>
                  )}
                </div>
              )}
            ...
            <button
              onClick={() => {
                const data = {
                  ...
                  scheduledAt: blogForm.scheduledAt
                    ? new Date(blogForm.scheduledAt).toISOString()
                    : undefined,
