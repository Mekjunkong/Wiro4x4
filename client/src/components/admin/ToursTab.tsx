...
export function ToursTab() {
  ...
  const createTourMut = trpc.tour.create.useMutation({
    onSuccess: () => { refetchTours(); setTourDialogOpen(false); resetTourForm(); toast.success('Tour created!'); },
    onError: (error) => { console.error('Failed to create tour:', error); toast.error('Failed to create tour.'); },
  });
  const updateTourMut = trpc.tour.update.useMutation({
    onSuccess: () => { refetchTours(); setTourDialogOpen(false); resetTourForm(); toast.success('Tour updated!'); },
    onError: (error) => { console.error('Failed to update tour:', error); toast.error('Failed to update tour.'); },
  });
  const deleteTourMut = trpc.tour.delete.useMutation({
    onSuccess: () => { refetchTours(); toast.success('Tour deleted!'); },
    onError: (error) => { console.error('Failed to delete tour:', error); toast.error('Failed to delete tour.'); },
  });
  ...
  return (
    <div className="p-6">
      ...
      {toursLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">No tours yet. Add your first tour.</div>
      ) : (
        <div className="space-y-4">
          {allTours.map(tour => (
            <div key={tour.id} className="border border-border rounded-lg overflow-hidden">
                ...
                <div className="flex items-start gap-3 md:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      ...
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${tour.isActive === 1 ? 'bg-green-100 text-green-800' : 'bg-muted text-foreground'}`}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={tourForm.name} onChange={e => setTourForm(p => ({ ...p, name: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Name (Hebrew) *</label>
                  <input type="text" dir="rtl" value={tourForm.nameHe} onChange={e => setTourForm(p => ({ ...p, nameHe: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Description (English) *</label>
                <textarea value={tourForm.description} onChange={e => setTourForm(p => ({ ...p, description: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" rows={2} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Description (Hebrew) *</label>
                <textarea dir="rtl" value={tourForm.descriptionHe} onChange={e => setTourForm(p => ({ ...p, descriptionHe: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" rows={2} />
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Duration *</label>
                  <input type="text" placeholder="e.g. 6-8 hours" value={tourForm.duration} onChange={e => setTourForm(p => ({ ...p, duration: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Difficulty</label>
                  <select value={tourForm.difficulty} onChange={e => setTourForm(p => ({ ...p, difficulty: e.target.value as any }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm">
                ...
                <div>
                  ...
                  <input type="number" value={tourForm.price} onChange={e => setTourForm(p => ({ ...p, price: parseInt(e.target.value) || 0 }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Min Group Size</label>
                  <input type="number" value={tourForm.groupMinSize} onChange={e => setTourForm(p => ({ ...p, groupMinSize: parseInt(e.target.value) || 1 }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Max Group Size</label>
                  <input type="number" value={tourForm.groupMaxSize} onChange={e => setTourForm(p => ({ ...p, groupMaxSize: parseInt(e.target.value) || 10 }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Image URL *</label>
                <input type="text" value={tourForm.imageUrl} onChange={e => setTourForm(p => ({ ...p, imageUrl: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" placeholder="/images/tour.jpg or https://..." />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Highlights (JSON array, English)</label>
                <textarea value={tourForm.highlights} onChange={e => setTourForm(p => ({ ...p, highlights: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" rows={2} placeholder='["Private 4x4 vehicle", "Kosher lunch", ...]' />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Highlights (JSON array, Hebrew)</label>
                <textarea dir="rtl" value={tourForm.highlightsHe} onChange={e => setTourForm(p => ({ ...p, highlightsHe: e.target.value }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" rows={2} placeholder='["&#1512;&#1499;&#1489; 4x4 &#1508;&#1512;&#1496;&#1497;", "&#1488;&#1512;&#1493;&#1495;&#1514; &#1510;&#1492;&#1512;&#1497;&#1497;&#1501; &#1499;&#1513;&#1512;&#1492;", ...]' />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Sort Order</label>
                  <input type="number" value={tourForm.sortOrder} onChange={e => setTourForm(p => ({ ...p, sortOrder: parseInt(e.target.value) || 0 }))} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
                </div>
              </div>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setTourDialogOpen(false); resetTourForm(); }} className="flex-1 px-4 py-2 border border-border rounded-lg text-sm hover:bg-muted/50">Cancel</button>
              <button onClick={() => {
